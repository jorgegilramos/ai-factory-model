...
class TestModelFactory(unittest.TestCase):
    ...
    @patch('ai_factory_model.llm.model_utils.load_from_file', autospec=True)
    def test_get_model_from_cache(self, mock_load_from_file):
        # Configurar el mock
        ...
        assert_that(retrieved_model).is_equal_to(model)
    ...
    def test_create_model(self, mock_azure_chat_openai, mock_azure_auth_client):
        ...
        assert_that(created_model.client.azure_endpoint).is_equal_to("https://example.com")
        assert_that(created_model.client.azure_ad_token).is_not_none()
        assert_that(created_model.client.deployment_name).is_equal_to("test_model")
        assert_that(created_model.client.openai_api_version).is_equal_to("1.0")
