...
class TestAzureOpenAIChatModelReal:

    def test_initialize_model_service_principal(self, env_testing):
        if env_testing:
    ...
    def test_initialize_model_api_key(self, env_testing):
        if env_testing:
    ...
    def test_initialize_model_invalid_auth(self, env_testing):
        if env_testing:
            config = {
                "model_name": "test_model",
                "model_version": "1.0",
    ...
    def test_create_model(self, env_testing):
        if env_testing:
            with patch("ai_factory_model.llm.model_AzureOpenAIChat.AzureAuthClient",
                 autospec=True) as mock_azure_auth_client:
    ...
    def test_read_model(self, env_testing):
        if env_testing:
    ...
    def test_update_model(self, env_testing):
        if env_testing:
    ...
    def test_delete_model(self, env_testing):
        if env_testing:
    ...
    def test_prompt_render(self, env_testing):
        if env_testing:
